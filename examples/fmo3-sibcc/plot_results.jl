...
population_direct = Float64.(readdlm(
    joinpath(outdir, "pop_heom_fmo62_direct_esprit.dat"); comments=true, comment_char='#',
))
...
lines!(ax_population, population_sibcc[:, 1], population_sibcc[:, 4];
       linewidth=3, label="rho33 SIBCC", color=:seagreen3)
...
lines!(ax_population, population_direct[:, 1], population_direct[:, 4];
       linewidth=2.5, linestyle=:dash, label="rho33 direct ESPRIT", color=:green4)
...
println("  ", joinpath(outdir, "bcf_fmo62_method_comparison_site3.png"))
